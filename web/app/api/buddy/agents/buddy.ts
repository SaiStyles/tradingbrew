...
export async function runBuddy(params: BuddyParams): Promise<string> {
  ...
  try {
    ...
    const system = [
      ...
      `\nBased on state:\nidle → chat naturally; address intervention first if flagged\nawaiting_trade_confirmation → confirm details, ask if right\nawaiting_entry_time → ask entry time, casual, one question\nawaiting_missing_fields → ask once about entry/exit prices for chart\nawaiting_emotion_confirmation → name inferred emotion, confirm it\nawaiting_execution_score → ask execution score 1-10\n\nRULES: never robotic, no "logged"/"saved"/"state", no memory refs, no signals/advice, one question max, never ask for fields already in pending, positive comparisons only, intervention naturally not like an alert. Be efficient — when multiple pieces of information are missing, ask for them naturally in one sentence rather than one at a time. Never ask for anything already present in pending_trade_data.\n\nPlain text only. No JSON. No markdown.`,
    ].filter(Boolean).join('\n')
